...
export const curatorVaultWithdrawal = onchainTable(
...
export const curatorVaultDepositRelations = relations(
	curatorVaultDeposit,
	({ one }) => ({
		assetVault: one(assetVault, {
			fields: [curatorVaultDeposit.assetVault],
			references: [assetVault.id],
		}),
	})
);

export const curatorVaultWithdrawalRelations = relations(
	curatorVaultWithdrawal,
	({ one }) => ({
		assetVault: one(assetVault, {
			fields: [curatorVaultWithdrawal.assetVault],
			references: [assetVault.id],
		}),
	})
);
...
export const assetVaultRelations = relations(assetVault, ({ one }) => ({
	curator: one(curator, {
		fields: [assetVault.curator],
		references: [curator.id],
	}),
}));

export const assetVaultAllocationRelations = relations(
	assetVault,
	({ many }) => ({
...
export const allocationRelations = relations(allocation, ({ one }) => ({
	assetVault: one(assetVault, {
		fields: [allocation.assetVault],
		references: [assetVault.id],
	}),
}));

export const curatorDeployedVaultsRelations = relations(
	curator,
	({ many }) => ({
